    ...
    <x-filament::section>
        ...
        <div class="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-6 gap-3">
            @foreach($actions as $action)
                ...
                <a href="{{ $action['url'] }}" 
                   class="group flex flex-col items-center justify-center p-3 rounded-lg bg-gradient-to-br {{ $action['gradient'] }} text-white shadow-sm hover:shadow-md transition-all duration-200 transform hover:scale-105 min-h-[80px]">
                    
                    {{-- Icon --}}
                    <div class="mb-1.5 flex items-center justify-center w-8 h-8 rounded-lg bg-white/20 backdrop-blur-sm group-hover:bg-white/30 transition-all duration-200">
                        @switch($action['icon'])
                            @case('heroicon-o-home-modern')
                                <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M3.75 21h16.5M4.5 3h15M8.25 3v18m7.5-18v18M7.5 3h9M3.75 9h16.5m-16.5 6h16.5"></path>
                                </svg>
                                @break
